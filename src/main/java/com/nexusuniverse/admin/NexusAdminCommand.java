...
public class NexusAdminCommand implements CommandExecutor {
    ...
    /**
     * nexusadmin.menu (default: op), OR a name listed in menu.priority-players -- see
     * AdminConfig#isPriorityPlayer for why the second path exists (a server owner shouldn't be
     * locked out of their own admin panel just because their permissions setup doesn't actually
     * grant them a "default: op" node).
     */
    private boolean canOpenMenu(Player player) {
        return player.hasPermission("nexusadmin.menu") || config.isPriorityPlayer(player.getName());
    }
